<?php
if ( ! class_exists( 'Original_Blog_Author_Info_Widget' ) ) {
	/**
	 * Adds Original Blog Author Info Widget.
	 */
	class Original_Blog_Author_Info_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$original_blog_author_info_widget = array(
				'classname'   => 'widget original-blog-widget author-widget',
				'description' => __( 'Retrive Author Information Widget', 'original-blog' ),
			);
			parent::__construct(
				'original_blog_author_info_widget',
				__( 'Artify Widget: Author Info Widget', 'original-blog' ),
				$original_blog_author_info_widget
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Section Title:', 'original-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name:', 'original-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'designation' ) ); ?>"><?php esc_html_e( 'Author Designation:', 'original-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'original-blog' ); ?></label>
			...
			<div>
				<label for="<?php echo esc_attr( $this->get_field_id( 'author_image_url' ) ); ?>"><?php esc_html_e( 'Author Image URL', 'original-blog' ); ?></label>:<br />
				...
				<div class="image-preview" style="<?php echo esc_attr( $style_attr ); ?>">
					<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php esc_attr_e( 'Preview', 'original-blog' ); ?>" style="width: 200px;"/>
				</div><!-- .image-preview -->
				<input type="button" class="select-img button" value="<?php esc_attr_e( 'Upload', 'original-blog' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'original-blog' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'original-blog' ); ?>" style="margin-top:5px;" /><br/>
			</div>
			<?php
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Link %s :', 'original-blog' ), $i ); ?></label>
 ...
 *
 * @since Original Blog 1.0
 **/
function original_blog_widget_image_upload_enqueue( $hook ) {
	...
	wp_enqueue_media();
	wp_enqueue_script( 'widget-image-upload-script', get_template_directory_uri() . '/assets/js/upload.min.js', array( 'jquery' ), ORIGINAL_BLOG_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'original_blog_widget_image_upload_enqueue' );
