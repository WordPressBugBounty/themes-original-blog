<?php
if ( ! class_exists( 'Original_Blog_Social_Widget' ) ) {
	/**
	 * Adds Original Blog Social Widget.
	 */
	class Original_Blog_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$original_blog_social_widget = array(
				'classname'   => 'widget original-blog-widget social-widget style-2',
				'description' => __( 'Retrive Social Widget', 'original-blog' ),
			);
			parent::__construct(
				'original_blog_social_widget',
				__( 'Artify Widget: Social Widget', 'original-blog' ),
				$original_blog_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Link %d :', 'original-blog' ), $i ); ?></label>
