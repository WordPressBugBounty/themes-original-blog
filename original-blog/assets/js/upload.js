...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var Original_Blog_Image = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    original_blog_file_frame = wp.media.frames.original_blog_file_frame = wp.media({
      button: {
        text: jQuery( this ).data( 'uploader_button_text' ),
      },
      state : 'author-info-insert-image',
      states : [
        new Original_Blog_Image()
        ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

    // When an image is selected, run a callback.
    original_blog_file_frame.on( 'select', function() {

      var state = original_blog_file_frame.state('author-info-insert-image');
